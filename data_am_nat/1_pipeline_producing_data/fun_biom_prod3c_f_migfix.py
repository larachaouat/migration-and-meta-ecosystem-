...
# This script
# - wraps repeated model runs of a 3-compartment ecosystem model (migration at a fixed time)
...
def calculate_tot_production_3C_migfix(I1, e1, a1, amp1, F, F1, F2, h1, h2, rc, rp, mp, mc1, mc2, nb_years, X0, f,  s, ext, B, Fmig=None, amp2=None, var1='I1', var2='a1', range1=None, range2=None):    # Set default ranges if none are provided
    """
    Calculate persistence and summary statistics over a 2D parameter grid for the 3-component ecosystem model
    with and without migration (migration "fixed" scenario).
    ...
    b_list_min = np.zeros((8, B, B))
    b_list_min_m = np.zeros((8, B, B))
    b_list_max = np.zeros((8, B, B))
    b_list_max_m = np.zeros((8, B, B))
...
#timing max
#same for extrema
            b_list_max[0,i,j] = np.max(N1[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max[1,i,j] = np.max(N2[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max[2,i,j] = np.max(P1[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max[3,i,j] = np.max(P2[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max[4,i,j] = np.max(C[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max[5,i,j] = np.max(Ca[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max[6,i,j] = np.max(C2[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max[7,i,j] = np.max(C1[int(t_end - 1 / f) * 10:t_end * 10 - 2])

            b_list_min[0,i,j] = np.min(N1[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min[1,i,j] = np.min(N2[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min[2,i,j] = np.min(P1[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min[3,i,j] = np.min(P2[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min[4,i,j] = np.min(C[int(t_end - 1 / f) * 10:t_end * 10 - 2]+Ca[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min[5,i,j] = np.min(Ca[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min[6,i,j] = np.min(C2[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min[7,i,j] = np.min(C1[int(t_end - 1 / f) * 10:t_end * 10 - 2])

            b_list_max_m[0,i,j] = np.max(N1_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max_m[1,i,j] = np.max(N2_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max_m[2,i,j] = np.max(P1_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max_m[3,i,j] = np.max(P2_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max_m[4,i,j] = np.max(C_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max_m[5,i,j] = np.max(Ca_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max_m[6,i,j] = np.max(C2_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_max_m[7,i,j] = np.max(C1_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])

            b_list_min_m[0,i,j] = np.min(N1_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min_m[1,i,j] = np.min(N2_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min_m[2,i,j] = np.min(P1_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min_m[3,i,j] = np.min(P2_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min_m[4,i,j] = np.min(C_m[int(t_end - 1 / f) * 10:t_end * 10 - 2]+Ca_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min_m[5,i,j] = np.min(Ca_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min_m[6,i,j] = np.min(C2_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])
            b_list_min_m[7,i,j] = np.min(C1_m[int(t_end - 1 / f) * 10:t_end * 10 - 2])

            it += 1
            print(it)


    return p_list, p1_list, p2_list, b_list, b_list_m, b_list_min, b_list_min_m, b_list_max, b_list_max_m
